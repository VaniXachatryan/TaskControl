...
class ProductService(IProductService):
    ...
    async def add(self, code: str, batch_number: int, batch_date: date) -> Result[ProductResult, str]:
        async with self.uow:
            is_any_code: bool = await self.uow.products.any_by_code(code=code)
            if is_any_code is True:
                return Failure(ProductErrors.is_exist)

            batch: Batch | None = await self.uow.batches.get_by_number_and_date(number=batch_number, date=batch_date)
            if batch is None:
                return Failure(BatchErrors.not_found)
